...
def naiveSoftmaxLossAndGradient(
    ...
    x = outsideVectors @ centerWordVec
    y_hat = softmax(x)
    y = np.zeros((x.size,))
    y[outsideWordIdx] = 1

    # loss = -outsideVectors[outsideWordIdx,:] @ centerWordVec + np.log(sum(np.exp(x)))
    loss = -np.log(y_hat[outsideWordIdx])

    gradCenterVec = outsideVectors.T @ (y_hat-y)

    gradOutsideVecs = (y_hat-y).reshape((-1,1)) * centerWordVec
...
def negSamplingLossAndGradient(
    ...
    negSampleWords = outsideVectors[negSampleWordIndices,:]
    u_o = outsideVectors[outsideWordIdx,:]
    # negSampleWordIndicesCounts = Counter(negSampleWordIndices)
    loss = -np.log(sigmoid(u_o @ centerWordVec)) - np.sum(np.log(sigmoid(-negSampleWords @ centerWordVec)))
    # print(negSampleWords.shape)
    # print(centerWordVec.shape)
    # print(np.sum((1-sigmoid(-negSampleWords @ centerWordVec.reshape((-1,1)))) * negSampleWords, axis = 0).shape)
    gradCenterVec = -(1-sigmoid(u_o @ centerWordVec)) * u_o + np.sum((1-sigmoid(-negSampleWords @ centerWordVec.reshape((-1,1)))) * negSampleWords, axis = 0)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx,:] = -(1-sigmoid(u_o @ centerWordVec)) * centerWordVec
    for neg_idx in negSampleWordIndices:
        # gradOutsideVecs[neg_idx,:] += negSampleWordIndicesCounts[neg_idx]*(1-sigmoid(-outsideVectors[neg_idx] @ centerWordVec)) * centerWordVec
        gradOutsideVecs[neg_idx,:] += (1-sigmoid(-outsideVectors[neg_idx] @ centerWordVec)) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss_o, gradCenterVec_o, gradOusideVecs_o = word2vecLossAndGradient(centerWordVec=centerWordVec, outsideWordIdx=outsideWordIdx, outsideVectors=outsideVectors, dataset=dataset)
        loss += loss_o
        gradCenterVecs[centerWordIdx] += gradCenterVec_o
        gradOutsideVectors += gradOusideVecs_o
    ### END YOUR CODE
